...
const getDefaultImageUrl = (firstName, lastName) => {
  const initials = `${firstName.charAt(0).toUpperCase()}${lastName.charAt(0).toUpperCase()}`;
  return `https://api.dicebear.com/5.x/initials/svg?seed=${initials}`;
};
...
//delete Account
//Explore -> how can we schedule this delwte operation
...
exports.updateDisplayPicture = async (req,res) => {
  try {
    ...
    if (!req.files || !displayPicture) {
      // If no file is provided, remove the profile picture by setting it to null or a default value
      const defaultImageUrl = getDefaultImageUrl(user.firstName, user.lastName);
      const updatedProfile = await User.findByIdAndUpdate(
        { _id: userId },
        { image: defaultImageUrl  }, // Set to null or a default image URL
        { new: true }
      );
