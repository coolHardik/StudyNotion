...
import { deleteCourse, fetchCourseDetails, fetchInstructorCourses } from "../../../../services/operations/courseDetailsAPI"
...
export default function CoursesTable({ courses, setCourses }){
  ...
  const getCourseTime = async(courseId) => {
    setLoading(true)
    try {
      let data = await fetchCourseDetails(courseId)
      const duration = data.data.totalDuration 
      setCourseDurations(prevState => ({
        ...prevState,
        [courseId]: duration,
      }))
    } catch (error) {
            ...
            courses?.map((course) => (
              <Tr key={course._id}  className="flex gap-x-10 border-b border-richblack-800 px-6 py-8" >
                ...
                <Td className="text-sm font-medium text-richblack-100">
                    {courseDurations[course._id] ? `${courseDurations[course._id]}` : "Loading..."}
                </Td>
